...
**协议品牌名：Troodon**（已定稿）。规范 CC BY 4.0、代码 Apache-2.0。**对外发布待项目决策。**
...
- **结算**：Mock / x402 / AP2 / 法币 HTTP 骨架（协议不碰资金）。
...
.\.venv\Scripts\python.exe -m pytest -q          # 208+ tests
.\.venv\Scripts\python.exe demo\run_demo.py
.\.venv\Scripts\python.exe demo\plugfest.py       # 8 场景冒烟
```
