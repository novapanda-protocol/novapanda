...
> 协议品牌名：**NovaPanda**（工作名，已定稿）。文字商标注册与官方域名筹备中；公开前见 [`../conformance/PRE_PUBLISH_CHECKLIST.md`](../conformance/PRE_PUBLISH_CHECKLIST.md)。
...
Agent SHOULD 在 `/.well-known/novapanda.json` 自签 manifest：

| 字段 | R/O | 说明 |
|------|:---:|------|
| `protocol` | R | `"novapanda"` |
| `version` | R | 规范版本 |
| `agent_id` | R | Ed25519 agent_id |
| `did` | O | `did:novapanda:<agent_id>`；若 present MUST 与 `agent_id` 一致 |
...
| `E_REPUTATION_LOW` | 403 | 信誉门槛未达（`NOVAPANDA_REP_MIN_SCORE`） |
...
默认 `NOVAPANDA_WITNESS_V2=0`（端点返回 501）。设为 `1` 后启用 attach/lock/slash/release。
...
`NOVAPANDA_WITNESS_REQUIRE_STAKE=1` 时 attach 前 MUST 存在同 exchange 的 locked stake。
...
| POST | `/v2/reputation/import` | 导入 mirror（`NOVAPANDA_FEDERATION_V2=1`） |
...
| 方法 | 路径 | 说明 |
|------|------|------|
| — | `NOVAPANDA_VERIFIER=llm` | LLM 验收；`NOVAPANDA_LLM_JUDGE=regex\|field_match\|http\|openai` |
| — | `NOVAPANDA_LLM_GATEWAY_URL` / `NOVAPANDA_LLM_MODEL` / `NOVAPANDA_LLM_API_KEY` | LLM 网关配置 |
...
交换流 MAY 用纯 DID：`propose.provider` / `client` 为 `did:novapanda:…`；`contract.party_did` + 签名。
...
环境变量（完整列表见 `novapanda/config.py`）：

| 变量 | 说明 |
|------|------|
| `NOVAPANDA_AUTH` | 鉴权开关 |
| `NOVAPANDA_DB` / `NOVAPANDA_REPUTATION_DB` | SQLite |
| `NOVAPANDA_SETTLEMENT` + `NOVAPANDA_*_URL` | 结算适配器 |
| `NOVAPANDA_WITNESS_V2` / `NOVAPANDA_FEDERATION_V2` | v2 特性 |
| `NOVAPANDA_TIMEOUT_*` | 各阶段超时 |
| `NOVAPANDA_VERIFIER` / `NOVAPANDA_LLM_*` | 验收器 |
| `NOVAPANDA_REP_WEIGHTS` / `NOVAPANDA_REP_MIN_SCORE` / `NOVAPANDA_REP_GATE_STRICT` | 信誉 gate |
| `NOVAPANDA_WITNESS_REQUIRE_STAKE` | witness 须先 stake |
| `NOVAPANDA_ARBITRATOR` | 独立仲裁者 |