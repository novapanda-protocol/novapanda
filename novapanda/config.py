...
协议品牌：**NovaPanda**（环境变量前缀 `NOVAPANDA_*`、包名 `novapanda` 保持不变）。
...
环境变量（均可选）：  NOVAPANDA_AUTH=1|0              默认 1（开启请求鉴权）
  ...
  NOVAPANDA_ARBITRATOR=agent_id   争议裁决者（设后 resolve 仅允许该身份）
  NOVAPANDA_WITNESS_V2=1|0        启用 witness v2 attach/slash（默认 0）
  NOVAPANDA_FEDERATION_V2=1|0     启用跨节点信誉导入（默认 0）
  NOVAPANDA_TIMEOUT_CONTRACT=sec  默认 contract 超时（默认 300）
  NOVAPANDA_TIMEOUT_ESCROW=sec      默认 escrow 超时（默认 300）
  NOVAPANDA_TIMEOUT_DELIVER=sec     默认 deliver 超时（默认 600）
  NOVAPANDA_TIMEOUT_VERIFY=sec      可选 verify 超时（DELIVERED 后须验收）
  NOVAPANDA_TIMEOUT_CONFIRM=sec     可选 confirm 超时（VERIFIED 后 client 须确认）
  NOVAPANDA_VERIFIER=schema|llm     验收器（默认 schema）
  NOVAPANDA_LLM_JUDGE=regex|field_match|http|openai  llm 验收器 judge（http/openai 需 LLM_GATEWAY_URL）
  NOVAPANDA_LLM_GATEWAY_URL=http://...  LLM 裁判 HTTP 网关（NOVAPANDA_LLM_JUDGE=http|openai）
  NOVAPANDA_REP_WEIGHTS=json        默认信誉加权，如 {"ed25519:NodeA":1.0}
  NOVAPANDA_REP_MIN_SCORE=float     propose/escrow 前 provider 信誉门槛（不设则关闭）
  NOVAPANDA_REP_GATE_STRICT=1|0     无信誉历史时也拒绝（默认 0）
  NOVAPANDA_WITNESS_REQUIRE_STAKE=1|0  attach witness 前须锁定 stake（默认 0）
  NOVAPANDA_LLM_MODEL=name          OpenAI-compat 模型名（NOVAPANDA_LLM_JUDGE=openai）
  NOVAPANDA_LLM_API_KEY=secret      OpenAI-compat Bearer（可选）
  NOVAPANDA_LLM_CONSENSUS=spec      多裁判共识，如 unanimous:regex,field_match
  NOVAPANDA_REP_WITNESS_BONUS=0.05  每条 locked stake 对 gate 的有效分加成
  NOVAPANDA_REP_WITNESS_BONUS_CAP=0.25  见证加成上限
  NOVAPANDA_X402_API_KEY=secret     x402 网关 Bearer（可选）
...
class NodeConfig:
    ...
    def from_env(cls) -> "NodeConfig":
        db = os.environ.get("NOVAPANDA_DB")
        rep = os.environ.get("NOVAPANDA_REPUTATION_DB") or db
        onto = os.environ.get("NOVAPANDA_ONTOLOGY")
        rules = os.environ.get("NOVAPANDA_RULES")
        timeouts = dict(DEFAULT_TIMEOUTS)
        for phase, env_name in (
            ("contract", "NOVAPANDA_TIMEOUT_CONTRACT"),
            ("escrow", "NOVAPANDA_TIMEOUT_ESCROW"),
            ("deliver", "NOVAPANDA_TIMEOUT_DELIVER"),
            ("verify", "NOVAPANDA_TIMEOUT_VERIFY"),
            ("confirm", "NOVAPANDA_TIMEOUT_CONFIRM"),
        ):
            raw = os.environ.get(env_name)
            if raw is not None:
                timeouts[phase] = int(raw)
        rep_weights: dict = {}
        raw_weights = os.environ.get("NOVAPANDA_REP_WEIGHTS")
        if raw_weights:
            rep_weights = json.loads(raw_weights)
        rep_min_raw = os.environ.get("NOVAPANDA_REP_MIN_SCORE")
        rep_min = float(rep_min_raw) if rep_min_raw is not None else None
        rep_bonus_raw = os.environ.get("NOVAPANDA_REP_WITNESS_BONUS")
        ...
        return cls(
            ...
            arbitrator_id=os.environ.get("NOVAPANDA_ARBITRATOR"),
            ap2_gateway_url=os.environ.get("NOVAPANDA_AP2_URL"),
            fiat_gateway_url=os.environ.get("NOVAPANDA_FIAT_URL"),
            witness_v2=_env_bool("NOVAPANDA_WITNESS_V2", False),
            federation_v2=_env_bool("NOVAPANDA_FEDERATION_V2", False),
            verifier=os.environ.get("NOVAPANDA_VERIFIER", "schema").lower(),
            llm_judge=llm_judge,
            llm_gateway_url=os.environ.get("NOVAPANDA_LLM_GATEWAY_URL"),
            reputation_weights=rep_weights,
            reputation_min_score=rep_min,
            reputation_gate_strict=_env_bool("NOVAPANDA_REP_GATE_STRICT", False),
            witness_require_stake=_env_bool("NOVAPANDA_WITNESS_REQUIRE_STAKE", False),
            llm_model=os.environ.get("NOVAPANDA_LLM_MODEL"),
            llm_api_key=os.environ.get("NOVAPANDA_LLM_API_KEY"),
